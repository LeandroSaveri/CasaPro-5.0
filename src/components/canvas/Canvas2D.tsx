...
const Canvas2D: React.FC = () => {
  ...
  const snapAngle = useCallback((angle: number): number => {
    ...
    return angle;
  }, [snapEnabled, projectElements?.settings.snapToAngle, projectElements<response clipped><NOTE>Result is longer than **10000 characters**, will be **truncated**.</NOTE>
      minDist = dist;
...
const render = useCallback(() => {
  ...
  projectElements.rooms.forEach((room: Room) => {
    ...
    renderRoom(ctx, room, isSelected, isHovered, worldToCanvas);
  });
  ...
  projectElements.walls.forEach((wall: Wall) => {
    ...
    renderWall(ctx, wall, isSelected, isHovered, worldToCanvas);
  });
  ...
  drawPreview({
    ...
    snapIndicator,
    calculateAngle,
    getD<response clipped><NOTE>Result is longer than **10000 characters**, will be **truncated**.</NOTE>
