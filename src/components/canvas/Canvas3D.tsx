...
const Scene: React.FC = () => {
  ...
  return (
    <>
      ...
      {/* Ambient Light */}
      <ambientLight 
        intensity={isNight ? 0.4 : lighting.ambientIntensity} 
        color={isNight ? '#5555aa' : lighting.ambientColor}
      />
      ...
      <div className="absolute bottom-4 left-4 p-3 bg-[#252532]/95 backdrop-blur-xl border border-white/20 rounded-xl">
        <div className="text-xs text-white/70 space-y-1">
          ...
          <div className="flex items-center gap-2">
            ...
            Sombras: {lighting.shadowsEnabled ? 'Ativadas' : 'Desativadas'}
          </div>
          <div className="flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-blue-400"></span>
            Exposição: {lighting.exposure.toFixed(1)}
          </div>
        </div>
      </div>
    </div>
